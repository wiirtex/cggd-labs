...
// #pragma omp parallel for default(none) shared(depth) shared(up)
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    float u = (2.f * float(x) + jitter.x) / static_cast<float>(width) - 1.f;
                    float v = (2.f * float(y) + jitter.y) / static_cast<float>(height) - 1.f;
